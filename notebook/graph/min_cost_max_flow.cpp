// Fuente: https://github.com/mhunicken/icpc-team-notebook-el-vasito/blob/master/flow/min_cost_max_flow.cpp
...
struct MCF{
	...
	void add_edge(int s, int t, tf cap, tc cost) {
		g[s].pb((edge){t,SIZE(g[t]),0,cap,cost});
		g[t].pb((edge){s,SIZE(g[s])-1,0,0,-cost});
	}
