...
ll dp(int i, int j, int k){ // i es la columna actual, j la fila y k la bitmask que describe la columna anterior
	if (i == m && k == 0) return 1; // Si termine de recorrer las filas y puse cosas validas en la fila anterior, entonces llegue a una configuracion valida
	if (i == m) return 0; // Si termine de recorrer las filas y puse cosas invalidas en la fila anterior, entonces llegue a una configuracion invalida
	if (j == n) return dp(i+1, 0, k); // Si termine la ultima fila, ahora me muevo a la siguiente columna
	...
	if (memo[i][j][k] == UNDEFINED){
		...
		if (k & a){ // Si estaba prendido el bit j en esta posicion quiere decir que comenzaba un bloque horizontal en (i-1, j). Luego, esta posicion esta ocupada y no puedo hacer nada
			memo[i][j][k] = dp(i, j+1, k & (~a)); // Apago el bit j del vector k y me voy una fila para abajo
		} else { // Habia puesto uno vertical, entonces esta posicion esta libre asi que tengo dos opciones: uno horizontal o uno vertical
			memo[i][j][k] = dp(i, j+1, k | a); // Pongo uno horizontal y me voy abajo
			// Me fijo si puedo poner uno vertical. Chequeo que el bit j+1 este apagado
			if (j+1 < n && (k & b) == 0){ //Si esta libre el bit en la posicion j+1
